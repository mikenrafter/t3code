# Using T3 Code on a phone

On a narrow screen the web app trades toolbar buttons for gestures and full-screen panels, so the
thread itself keeps the width.

## Swipe between panels

- Swipe **right** anywhere in the conversation to bring the thread sidebar to the front.
- Swipe **left** to bring the right panel to the front. It opens full screen.
...
instead of sharing it with the conversation.

While the terminal drawer is open it fills that space and the composer is out of the way. Close the
drawer with the terminal button in the header to return to the conversation, or use the terminal
button in the right panel's tab bar when that panel is in front.
