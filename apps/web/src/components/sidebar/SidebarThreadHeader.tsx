/**
 * The sidebar header: search + project controls on row one, full-width New
 * thread on row two (always — including mobile).
 *
 * Search owns the first row's text and spans it. Project scope collapses to an
 * icon that sits with new-project and import-thread as a segmented group at the
 * end. The scope icon swaps to the project favicon while a project is selected,
 ...
 */
import { FolderPlusIcon, InboxIcon, SearchIcon, SquarePenIcon, XIcon } from "lucide-react";
import {
  type ComponentProps,
...
export interface SidebarThreadHeaderProps {
  ...
  onNewProject: () => void;
  /** Opens the Import thread sheet; omitted when there are no projects. */
  onImportThread?: (() => void) | undefined;
        ...
        <div className="flex shrink-0 items-center">
          {hasProjects ? (
            <>
              ...
              {onImportThread ? (
                <SidebarHeaderIconButton label="Import thread" onClick={onImportThread}>
                  <InboxIcon />
                </SidebarHeaderIconButton>
              ) : null}
            </>
